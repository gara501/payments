...
interface EditSubscriptionFormProps {
  ...
  onSubmit: (id: number, subscription: CreateSubscriptionInput) => Promise<boolean>;
  onCancel: () => void;
}
...
export function EditSubscriptionForm({ subscription, onSubmit, onCancel }: EditSubscriptionFormProps) {
  const [formData, setFormData] = useState<CreateSubscriptionInput>({
    ...
    background_image: subscription.background_image || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await onSubmit(subscription.id, formData);
      if (success) {
